...
import { AdminLoadCardProps } from './types'
...
export default function AdminLoadCard({ load, onDelete }: AdminLoadCardProps) {
    ...
    const removeLoad = async () => {
        try {
            ...
            setSnackBarState({ message: error.message || "Unable to delete the load.", severity: "error" })
        } finally {
    ...
    return (
        <Grid container direction="row" justifyContent="space-between" alignItems="center" gap='10px' sx={{ minHeight: '56px', margin: '5px 0' }}>
                ...
                <Grid container justifyContent="flex-start" alignItems="center" gap='10px'>
                    <Grid item>
                        <Grid container direction="column" justifyContent="space-between" alignItems="flex-start" sx={{
                            ...
                            <Grid item>
                                <span className='text-16'>{load.origin + " - " + load.destination}</span>
                            </Grid>
                            <Grid item>
                                <span className='text-12'>Equipment: {Equipment[load.equipment]}</span>
                            </Grid>
                        </Grid>
                    </Grid>
                </Grid>
            ...
            {confirmDialogOpen && <AppDeleteConfirmDialog onCancel={() => setConfirmDialogOpen(false)} onDelete={removeLoad} />}
            <Snackbar
