...
export default function AdminBrokerSortFieldBar({ fields }: SortFieldBarProps): JSX.Element {
    const { brokerSearchParams } = useTypedSelector(state => state.broker)
    const { setBrokerPage, setBrokerSort, setBrokerSortfield } = useActions()
    ...
    const onSelectHandler = (field: string) => {
        setBrokerPage(1)
        if (field === brokerSearchParams.sortField) {
            brokerSearchParams.order === OrderType.Ascending ? setBrokerSort(OrderType.Descending) : setBrokerSort(OrderType.Ascending)
        } else {
                    ...
                    fields.map(field =>
                        <MenuItem key={field} 
                            ...
                            onClick={() => {
                                ...
                                &nbsp;
                                {
                                    field === brokerSearchParams.sortField ?
                                    brokerSearchParams.order === OrderType.Ascending ?
