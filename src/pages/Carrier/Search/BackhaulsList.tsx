import { useEffect, useState } from "react"
import { useTypedSelector } from "../../../hooks/useTypedSelector"
import { IImportLoad } from "../../../types/importload"
import { BackhaulsProps } from "./types"
import { ICity } from "../../../types/city"
import { searchImportLoadsAxios } from "../../../api/importload"
import { getCityByFullnameAxios } from "../../../api/city"
import { Modal, Box, Grid, CircularProgress } from "@mui/material"
import CloseIcon from '@mui/icons-material/Close'
import ErrorMessage from "../../../components/Messages/ErrorMessage"
import MuiButton from "../../../components/Button/MuiButton"
import SearchCard from "./SearchCard"

const style = {
    position: 'absolute' as 'absolute',
    padding: '32px 16px',
    top: '50%',
    left: '50%',
    display: 'block',
    maxHeight: '80%',
    transform: 'translate(-50%, -50%)',
    bgcolor: 'background.paper',
    boxShadow: 24,
    minWidth: '20rem',
    width: '80%',
    maxWidth: '60rem',
    overflowY: 'scroll'
};

export default function BackhaulsList({ importLoad, truck, isToAnywhere, onClose }: BackhaulsProps) {
    
    const { importLoadSearchParams } = useTypedSelector(state => state.importLoad)
    const [backhaulsState, setTBackhaulsState] = useState<IImportLoad[]>([])
    const [loadingState, setLoadingState] = useState<boolean>(false)
    const [errorState, setErrorState] = useState<null | string>(null)

    const fetchBackhauls = async () =>{
        try {
            setLoadingState(true)

            const originCity = await getCityByFullnameAxios(importLoad.destination)
            let destinationCity: ICity | null = null
            if(!isToAnywhere) destinationCity = await getCityByFullnameAxios(importLoad.origin)

            const result = await searchImportLoadsAxios({
                pageSize: 25,
                currentPage: 1,
                searchCriteria: '',
                userId: '',
                includeNavProperties: true,
                itemList: [],
                pageCount: 0,
                totalItemsCount: 0,
                origin: originCity,
                destination: destinationCity,
                truck: truck,
                pickupStartDate: importLoad.delivery,
                deadhead: importLoadSearchParams.deadhead,
                milesMin: importLoadSearchParams.milesMin,
                milesMax: importLoadSearchParams.milesMax,
                sortField: importLoadSearchParams.sortField,
                order: importLoadSearchParams.order,
            })
            setTBackhaulsState(result.itemList)
        } catch (error) {
            setErrorState(error.message)
        } finally {
            setLoadingState(false)
        }
    };

    useEffect(() => {
        fetchBackhauls()
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [])

    return (
        <Modal open={true} onClose={onClose}>
            <Box sx={style}>
                <CloseIcon
                    sx={{ cursor: 'pointer', fill: 'var(--lightGrey)', '&:hover': { fill: "var(--red)" }, position: "absolute", top: "15px", right: "15px" }}
                    onClick={onClose}
                />
                <Grid container direction="column" spacing={2} sx={{ padding: '20px' }}>
                    <Grid item>
                        <p className="text-20" style={{ textAlign: "center" }}>
                            {
                                isToAnywhere ? 
                                `Backhauls from ${importLoad.destination} to anywhere` :
                                `Backhauls from ${importLoad.destination} to ${importLoad.origin}`
                            }
                        </p>
                        <p className="text-14" style={{ textAlign: "center" }}>
                            Up to 25 best loads based on current search settings. 
                            To see full available loads please use the main search panel.
                        </p>
                        {/* ImportLoads List */}
                        {
                            backhaulsState.length > 0 ?
                                <div style={{ margin: "30px 0" }}>
                                    {backhaulsState.map(backhaul => 
                                        <SearchCard 
                                            key={backhaul.id} importLoad={backhaul} 
                                            isBackhaul={true}
                                        />
                                    )}
                                </div> :
                                errorState ? <ErrorMessage appearance="regular">{errorState}</ErrorMessage> :
                                    <p style={{ padding: "0 15px" }}>
                                        {loadingState && <CircularProgress size="1rem" sx={{ mr: '10px' }} />} Waiting for backhauls.
                                    </p>
                        }
                    </Grid>
                    <Grid item sx={{ textAlign: 'right' }}>
                        <MuiButton variant="contained" onClickHandler={onClose}>
                            <span className="text-14" style={{color: 'var(--lightgreywhite)'}}>Close</span>
                        </MuiButton>
                    </Grid>
                </Grid>
            </Box>
        </Modal>
  )
}
